...
class JapaneseWritingApp:
    def __init__(self):
        ...
        # Initialize MangaOCR in the background
        try:
            logger.info("Pre-initializing MangaOCR")
            from manga_ocr import MangaOcr
            self.mocr = MangaOcr()
            logger.info("Successfully initialized MangaOCR")
        except Exception as e:
            logger.error(f"Failed to initialize MangaOCR: {str(e)}")
            self.mocr = None

    def create_study_session(self, group_id):
    ...
    def load_vocabulary(self):
        ...
        try:
            ...
            self.study_session_id = self.create_study_session(group_id)
    ...
    def grade_submission(self, image):
        ...
        try:
            # Initialize MangaOCR if not already initialized
            if self.mocr is None:
                try:
                    ...
                    logger.error(f"Failed to initialize MangaOCR: {str(e)}")
                    return "Error: Could not initialize OCR", "Error", "C", "Failed to initialize OCR system. Please try again or contact support."
            ...
            try:
                ...
                return "Error: Transcription failed", "Error", "C", f"Failed to transcribe image: {str(e)}"
            ...
            if self.study_session_id and self.current_word:
                try:
                    review_url = f"http://localhost:5001/study_sessions/{self.study_session_id}/review"
                    review_data = {
                        'word_id': self.current_word.get('id'),
                        'correct': grade in ['S', 'A'],  # Consider S and A grades as correct
                    ...
                    review_response = requests.post(review_url, json=review_data, headers=review_headers)
                    if review_response.status_code == 201:
                        logger.info("Successfully submitted review")
                    else:
                        logger.error(f"Failed to submit review. Status: {review_response.status_code}")
